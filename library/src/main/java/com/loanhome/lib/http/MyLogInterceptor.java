...
public class MyLogInterceptor implements Interceptor {
    ...
    public MyLogInterceptor(LoggingLevel level)
    {
        this(level, Logger.DEFAULT);
    }

    public MyLogInterceptor(LoggingLevel level, Logger logger)
    {
        this.level = level;
        if(this.level == null)
        {
     ...
     *
     * @param level 新的拦截等级
     * @return {@link MyLogInterceptor}
     */
    public MyLogInterceptor setLevel(LoggingLevel level)
    {
        this.level = level;
        if(this.level == null)
        {
...
//                break;
    ...
    private String getHttpStateString(Request request, Response response, long tookMs, Chain chain)
    {
        ...
        return String.format(Locale.CHINA.getDefault(),
                "[%s %d %s][%s %dms] %s",
                request.method(),
                response.code(),
                response.message(),
    ...
    /**
     * {@link MyLogInterceptor} 中使用到的log打印等级，包含7个log类型，有 {@link #NONE}, {@link #URL_BODY}, {@link #SINGLE},
