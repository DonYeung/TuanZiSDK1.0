...
//    }
    ...
    public static String errorType2HumanStr(IDCardQualityResult.IDCardResultType type, IDCardAttr.IDCardSide side) {
        String result = null;
        switch (type) {
			case IDCARD_QUALITY_FAILED_NOTIDCARD:
				result = "没有检测到身份证";
				break;
			case IDCARD_QUALITY_FAILED_NOTCLEAR:
                result = "请点击屏幕对焦";
                break;
			case IDCARD_QUALITY_FAILED_HAVEHIGHLIGHT:
                result = "太亮";
                break;
			case IDCARD_QUALITY_FAILED_HAVESHADOW:
                result = "太暗";
                break;
            case IDCARD_QUALITY_FAILED_NOTINBOUND:
                result = "请将身份证置于提示框内";
                break;
            case IDCARD_QUALITY_FAILED_CONVERT:
                result = "请将身份证摆正";
                break;
            case IDCARD_QUALITY_FAILED_NEEDFRONT:
                ...
                break;
			case IDCARD_QUALITY_FAILED_NEEDBACK:
				if (side == IDCardAttr.IDCardSide.IDCARD_SIDE_BACK)
					result = "请翻到国徽面";
				else {
					result = "请翻到人像面";
    ...
    public static byte[] readModel_New(Context context){
		InputStream inputStream = null;
		ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
		byte[] buffer = new byte[1024];
		try {
			int count = -1;
			inputStream = context.getResources().openRawResource(R.raw.meg_idcard);
